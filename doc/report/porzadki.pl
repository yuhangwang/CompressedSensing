#! /usr/bin/perl
# Script that cleans up tex files. This is modified version of file written
# long ago by Polish TeX hackers.
#
...
sub PrzeformatujAkapit {
        s/$Odstep*\\\\/\ \\\\/g;
        my @Wiersz = split;
        my $IloscSlow = scalar(@Wiersz);
        my $DlugoscWiersza = length($Wiersz[0]);
        for (my $i=1; $i<$IloscSlow; $i++) {
                $DlugoscWiersza += length($Wiersz[$i]) + 1;
                if (($DlugoscWiersza>70) || ($Wiersz[$i-1]=~/\\\\/)) {
                        $Wiersz[$i-1] .= "\n";
                        $DlugoscWiersza = length($Wiersz[$i]);
                };
        };
        $_ = join (" ", @Wiersz, "\n");
        s/$Odstep\\\\/\\\\/g;
